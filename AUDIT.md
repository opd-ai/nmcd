...
- [ ] **HIGH-3: GetNameUTXO always fetches output index 0** — `namedb/utxo.go:217-220` — Logic/API Contract — `GetNameUTXO` ignores `record.OutIndex` and always calls `GetUTXO(&record.TxHash, 0)`. Names stored at non-zero output indices will get wrong UTXO lookups or false "not found" errors. — **Remediation:** Call `GetUTXO(&record.TxHash, record.OutIndex)`. Validate with `go test -race ./namedb/...`.
...
- [ ] **HIGH-5: Rollback paths discard database errors** — `chain/blockchain.go:1779, 1809, 1816-1820, 1839-1844` — Error Handling — Reorg rollback operations (e.g., `RemoveLastHistoryEntry`) discard errors and continue. A failed rollback silently leaves the name database inconsistent. — **Remediation:** Propagate or aggregate rollback failures and abort claiming success when DB operations fail. Validate with `go test -race ./chain/...`.

- [ ] **HIGH-6: Batch writer stores caller-owned pointers** — `namedb/batch.go:55-56, 73-74, 81-83, 90-91` — Data Aliasing/API Contract — `BatchWriter` queues caller-owned pointers and writes them during `Commit`. Caller mutation between enqueue and commit changes what gets persisted, causing silent corruption. — **Remediation:** Copy inputs when enqueueing, or document ownership transfer explicitly. Validate with `go test -race ./namedb/...`.

- [ ] **HIGH-7: Sync peer never reselected on disconnect** — `network/sync.go:93-97, 111, 222-229` + `network/peermgr.go:203-212, 263-273` — Logic/Availability — When the selected sync peer disconnects, `syncPeer` remains non-nil, so sync never reselects another peer and IBD stalls indefinitely. — **Remediation:** On peer disconnect, clear/reselect `syncPeer`/`bestPeer`; gate sync operations on peer connectivity. Validate with `go test -race ./network/...`.

- [ ] **HIGH-8: HandleHeaders accepts unvalidated headers from any peer** — `network/sync.go:149-199, 204-214` — Security/Logic — `HandleHeaders` processes headers from any peer without validating header linkage, PoW, or timestamps. Any peer can spam bogus headers and trigger large numbers of useless block requests. — **Remediation:** Only accept headers from the active sync peer and validate header chain before requesting blocks. Validate with `go test -race ./network/...`.

- [ ] **HIGH-9: Mempool stores mutable transaction pointers** — `network/mempool.go:167-171, 203-208, 229-236` — Data Aliasing/API Contract — `AddTx` stores the caller's `*wire.MsgTx` directly; `GetTx`/`GetAll` return internal pointers. External mutation can corrupt mempool state and create races during relay. — **Remediation:** Deep-copy transactions on insert and on return using `tx.Copy()`. Validate with `go test -race ./network/...`.

### MEDIUM

- [ ] **MEDIUM-1: getinfo returns compact target bits as "difficulty"** — `rpc/server.go:527-532` — Logic/API Contract — The `getinfo` RPC returns `best.Bits` (compact target encoding) as the `"difficulty"` field. Clients expect a human-readable difficulty ratio. — **Remediation:** Compute actual difficulty ratio from compact bits. Validate with `go test -race ./rpc/...`.

- [ ] **MEDIUM-2: name_history dereferences blockchain without guard** — `rpc/server.go:1111-1125` — Nil Safety/Initialization — `nameHistory` dereferences `s.blockchain` without calling `requireBlockchain` first. A server created without a blockchain will panic. — **Remediation:** Add `requireBlockchain` guard at the start of `nameHistory`. Validate with `go test -race ./rpc/...`.

- [ ] **MEDIUM-3: lookupActiveNameRecord off-by-one in expiration check** — `rpc/server.go:772-775` — Logic/Boundary — Code treats `ExpiresAt <= bestHeight` as expired, but project convention is `ExpiresAt < currentHeight`. Names are incorrectly expired one block early. — **Remediation:** Change to `record.ExpiresAt < bestHeight`. Validate with `go test -race ./rpc/...`.
...
- [ ] **MEDIUM-5: Global defaultLogger has unsynchronized read/write** — `internal/logging/logger.go:160-178` — Concurrency — `defaultLogger` is read in `GetDefault()` and written in `SetDefault()` without synchronization. — **Remediation:** Guard with a mutex or use `atomic.Pointer`. Validate with `go test -race ./internal/logging/...`.
...
- [ ] **MEDIUM-7: findNameNewUTXOIndex returns 0 on failure** — `rpc/server.go:1053-1061` — Logic — Returns `0` (a valid index) when no NAME_NEW UTXO is found, causing `name_firstupdate` to build an invalid transaction instead of failing. — **Remediation:** Return `-1` or an error value and reject the RPC before tx creation. Validate with `go test -race ./rpc/...`.
...
- [ ] **MEDIUM-9: ExtractChainIDFromVersion uses signed shift** — `chain/auxpow.go:401-405` — Arithmetic/Logic — `int32` right-shift sign-extends negative versions, producing wrong chain IDs for versions with high bit set. — **Remediation:** Use `uint32(version) >> 16`. Validate with `go test -race ./chain/...`.
