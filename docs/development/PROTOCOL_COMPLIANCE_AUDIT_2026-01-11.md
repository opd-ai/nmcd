...
| Max Value Size | 520 bytes (consensus) | 1023 bytes | ⚠️ | `config/config.go:27` |
...
**Note on Max Value Size:** Namecoin Core enforces 520 bytes for standard relay policy but allows larger values at consensus level. nmcd uses 1023 bytes which is more permissive. This should be reviewed for strict compatibility.
