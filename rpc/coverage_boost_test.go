...
// Should fail because wallet has no addresses (no UTXOs)
if resp.Error == nil {
t.Log("nameNew succeeded unexpectedly - may have broadcast")
}
}
...
resp := server.nameNew(req)
// Should fail because there are no UTXOs to fund the transaction
if resp.Error == nil {
t.Log("nameNew succeeded - may have broadcast")
} else if resp.Error.Code != -6 && resp.Error.Code != -5 {
t.Logf("Got error code %d: %s", resp.Error.Code, resp.Error.Message)
}
}

// TestNameListNilBlockchainPanics tests nameList panics gracefully when blockchain is nil.
func TestNameListNilBlockchainPanics(t *testing.T) {
server := &Server{
blockchain:     nil,
rateLimiter:    newRateLimiter(defaultRateLimit),
maxRequestSize: defaultMaxRequestSize,
...
defer func() {
// Expected to panic since nameList doesn't nil-check blockchain
recover()
}()
_ = server.nameList(req)
}
